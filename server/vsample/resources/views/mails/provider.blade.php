        ...
        <tr>
            <td align="center">
                ...
                <table width="600" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff; border-radius:6px; overflow:hidden;">
                    ...
                    <!-- Greeting -->
                    <tr>
                        <td style="padding:20px; font-size:15px; color:#333; line-height: 1.5rem;">
                            <p style="font-weight: bold; font-size: 18px;">
                                Dear {{ $user?->name ?? 'User' }},
                            </p>
                            <p>
                                ( {{ $items }} ) Items {{ ucfirst($event) }} from <strong>{{ $name }}</strong> .

                                @if($entity)
                                    Entity: <strong>{{ ucfirst($entity ?? 'all') }}</strong> 
                                    @if($entityId) (#{{ $entityId }}) @endif
                                @endif
                            </p>
                            @if($fromMe)
                                <p style="color:#666; font-size:13px;">
                                    *This update was triggered by your supplier.*
                                </p>
                            @endif
                        </td>
                    </tr>

<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration {

    public function up () {

        Schema::create('coupons', function ( Blueprint $table ) {
            $table->id();
            $table->integer('store_id')->default(0);
            $table->integer('ref_id')->default(0);
            $table->integer('country_id')->default(0);
            $table->integer('city_id')->default(0);
            $table->integer('category_id')->default(0);
            $table->integer('game_id')->default(0);
            $table->integer('product_id')->default(0);
            $table->integer('campaign_id')->default(0);

            $table->string('code')->nullable();
            $table->integer('points')->default(0);
            $table->enum('type', ['public', 'private', 'campaign', 'register', 'birth_date', 'order_complete', 'level_up'])->default('public');

            $table->enum('discount_type', ['fixed', 'percentage'])->default('percentage');
            $table->decimal('discount', 10, 2)->default(0);
            $table->decimal('max_discount', 10, 2)->default(1000);
            $table->decimal('min_price', 10, 2)->default(0.01);
            $table->decimal('max_price', 10, 2)->default(100000);
            $table->integer('max_usages')->default(100);
            $table->integer('max_uses')->default(5);
            $table->integer('min_orders')->default(0);
            $table->integer('min_level')->default(0);
            $table->json('conditions')->nullable();

            $table->json('name')->nullable();
            $table->json('description')->nullable();
            $table->string('notes')->nullable();

            $table->boolean('active')->default(true);
            $table->timestamp('expires_at')->nullable();
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['store_id', 'code', 'deleted_at']);
        });

    }

};
